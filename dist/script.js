...
var Customizator = /*#__PURE__*/function () {
  ...
  _createClass(Customizator, [{
    ...
    value: function onScaleChange() {
      ...
      this.btnBlock.addEventListener("click", function (event) {
        if (event) {
          _this.scale = +event.target.value.replace(/x/g, "");
        }

        _this.recursy(_this.body); //2 вызов


        localStorage.setItem("scale", _this.scale);
      });
      ...
      body.childNodes.forEach(function (node) {
        if (node.nodeName === "#text" && node.nodeValue.replace(/\s+/g, "").length > 0) {
          if (!node.parentNode.getAttribute("font-size")) {
            ...
            node.parentNode.style.fontSize = node.parentNode.getAttribute("font-size") * _this2.scale + "px";
          } else {
            node.parentNode.style.fontSize = node.parentNode.getAttribute("font-size") * _this2.scale + "px";
          }
        } else {
    ...
    value: function onColorChange() {
      ...
      this.colorPicker.addEventListener("input", function (event) {
        _this3.color = event.target.value;
        _this3.body.style.backgroundColor = _this3.color;
        localStorage.setItem("color", _this3.color);
      });
    ...
    value: function onCear() {
      ...
      this.clear.addEventListener("click", function () {
        ...
        _this4.onScaleChange();

        _this4.onColorChange();
      });
    }
  }, {
    key: "injectStyle",
